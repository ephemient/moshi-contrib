...
class JsonSubtypesCodeGenerator : AbstractProcessor() {
    ...
    private fun generateFileSpec(
        ...
        val typeSpec = if (factoryName.isEmpty()) {
            generateAdapterTypeSpec(name, setOf(type.visibilityModifier), type, subtypes, true)
                .toBuilder()
                .addType(generateFactoryTypeSpec("Factory", emptySet(), name, type, subtypes))
                .build()
        } else {
            generateFactoryTypeSpec(name, setOf(type.visibilityModifier), "Adapter", type, subtypes)
                .toBuilder()
                .addType(generateAdapterTypeSpec("Adapter", setOf(KModifier.PRIVATE), type, subtypes, false))
                .build()
        }
    ...
    private val javaVisibilityModifiers = mapOf(
        Modifier.PUBLIC to KModifier.PUBLIC,
        Modifier.PROTECTED to KModifier.PROTECTED,
        Modifier.DEFAULT to KModifier.PRIVATE,
        Modifier.PRIVATE to KModifier.PRIVATE
    )
    private val kotlinVisibilityModifiers = mapOf(
        Flag.IS_PUBLIC to KModifier.PUBLIC,
        Flag.IS_PROTECTED to KModifier.PROTECTED,
        Flag.IS_INTERNAL to KModifier.INTERNAL,
        Flag.IS_PRIVATE to KModifier.PRIVATE
    )
    @OptIn(KotlinPoetMetadataPreview::class)
    private val TypeElement.visibilityModifier: KModifier
        get() {
            ...
            while (true) {
                for (modifier in element.modifiers) {
                    val elementVisibility = javaVisibilityModifiers[modifier] ?: continue
                    val index = kotlinVisibilityModifiers.values.indexOf(elementVisibility)
                    if (index > visibilityIndex) {
                        visibility = elementVisibility
                        visibilityIndex = index
                    }
                }
                getAnnotation(Metadata::class.java)?.run {
                    val flags = toImmutableKmClass().flags
                    for (
                        (index, entry) in kotlinVisibilityModifiers.entries.withIndex()
                            .drop(visibilityIndex.coerceAtLeast(0))
                    ) {
                        if (entry.key(flags)) {
            ...
            return visibility ?: KModifier.PUBLIC
        }
